...
public class TitleItemDecoration extends XRecyclerView.ItemDecoration {
    private static final String TAG = "TitleItemDecoration";
    private List<BaseRecyclerBean> mData;
    ...
    public TitleItemDecoration(Context context, List<BaseRecyclerBean> data) {
        super();
        mData = data;
        mPaint = new Paint();
        mBounds = new Rect();
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (parent instanceof XRecyclerView) {
                position = position - ((XRecyclerView) parent).getHeadSize();
            }
            if (position > -1) {
                if (position == 0) {
                    //等于0的时候绘制title
    ...
    public void onDrawOver(Canvas c, final RecyclerView parent, RecyclerView.State state) {
        ...
        int headSize=0;
        if(parent instanceof XRecyclerView){
            headSize=((XRecyclerView) parent).getHeadSize();
            if(position-headSize<0){
               return;
            }
        }
        //在搜索到没有的索引的时候position可能等于-1，所以在这里判断一下
        if (position == -1) {
            return;
        }

        String tag = mData.get(position-headSize).getLetters();
        View child = parent.findViewHolderForLayoutPosition(position).itemView;
        //Canvas是否位移过的标志
        boolean flag = false;
        if ((position-headSize + 1) < mData.size()) {
            //当前第一个可见的Item的字母索引，不等于其后一个item的字母索引，说明悬浮的View要切换了
            if (null != tag && !tag.equals(mData.get(position -headSize+ 1).getLetters())) {
